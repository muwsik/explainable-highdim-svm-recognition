...
out = rf"C:\Users\Victory\.vscode\repos\explainable-highdim-svm-recognition\charts\time_vs_processes_{size}.svg"
...
hp = hp[hp.n_proc.isin([1,2,4,6,8,10])]
hp10,hp500 = hp[hp.n_splits.eq(10)],hp[hp.n_splits.eq(500)]

bg = bg[bg.n_proc.isin([1,2,4,6,8,10]) & bg.max_feats.eq(500) & bg.n_est.eq(200)]
...
def add(d,name,col,dash="solid"):
    fig.add_trace(go.Scatter(x=d.n_proc,y=d.tr_time_mean,mode="lines+markers",name=name,
        line=dict(color=colors[name],width=1,dash=dash),
        marker=dict(size=4,color=colors[name],symbol="circle" if name=="HP-DSC" else "square"),
...
fig.update_xaxes(title_text="Number of processes",tickmode="array",tickvals=[1,2,4,6,8,10],
